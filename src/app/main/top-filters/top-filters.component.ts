...
export class TopFiltersComponent implements OnInit {
    ...
    if (this.name == true) {
      ...
      if (this.flag == null) {
        filtered_cards = this.employees_local.filter(function (card: any) {
          return card.FirstName.toLowerCase().startsWith(e);
        });
      } else {
        filtered_cards = this.employees_local.filter((card: any) => {
          return (
            card.FirstName.toLowerCase().startsWith(e) &&
            card.FirstName.startsWith(String.fromCharCode(this.flag + 65))
          );
        });
      }
      ...
      if (this.flag == null) {
        filtered_cards = this.employees_local.filter(function (card: any) {
          return card.Department.toLowerCase().startsWith(e);
        });
      } else {
        filtered_cards = this.employees_local.filter((card: any) => {
          return (
      ...
      if (this.flag == null) {
        filtered_cards = this.employees_local.filter(function (card: any) {
          return card.JobTitle.toLowerCase().startsWith(e);
        });
      } else {
        console.log(456);
        filtered_cards = this.employees_local.filter((card: any) => {
  ...
  ngOnInit(): void {
    ...
    this.empService.flag_subject.subscribe((value) => {
      this.flag = value;
    });
  }
}
